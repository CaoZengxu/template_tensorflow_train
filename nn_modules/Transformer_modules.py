...
def point_wise_feed_forward_network(d_model, diff,activation='relu'):
    return tf.keras.Sequential([
        tf.keras.layers.Dense(diff, activation=activation),
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, n_heads, ddf, dropout_rate=0.1,activation='relu'):
        super(EncoderLayer, self).__init__()
        
        self.mha = MutilHeadAttention(d_model, n_heads)
        self.ffn = point_wise_feed_forward_network(d_model, ddf,activation=activation)
...
class Encoder(layers.Layer):
    def __init__(self, n_layers, d_model, n_heads, ddf,
                 input_vocab_size, max_seq_len, drop_rate=0.1, activation='relu'):
        ...
        self.encode_layer = [EncoderLayer(d_model, n_heads, ddf, drop_rate,activation=activation)
    ...
    def call(self, inputs, training, mark):
        ...
        word_emb *= tf.sqrt(tf.cast(self.d_model, tf.float32))
...
if __name__ =='__main__':
    ...
    tmp = np.random.randint(low=0,high=100,size=[64,120])
    sample_encoder_output = sample_encoder(tf.constant(tmp),
                                           False, None)
